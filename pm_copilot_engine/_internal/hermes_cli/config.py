...
from pm_copilot_engine._internal.hermes_cli.default_soul import DEFAULT_SOUL_MD
...
# Re-export from pm_copilot_engine.hermes_constants — canonical definition lives there.
from pm_copilot_engine.hermes_constants import get_hermes_home  # noqa: F811,E402
from pm_copilot_engine.utils import atomic_replace
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    """
    try:
        from pm_copilot_engine.agent.skill_utils import discover_all_skill_config_vars, SKILL_CONFIG_PREFIX
    except Exception:
        return []
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        if answer in {"y", "yes"}:
            ...
            config = load_config()
            try:
                from pm_copilot_engine.agent.skill_utils import SKILL_CONFIG_PREFIX
            except Exception:
...
def show_config():
    ...
    # Skill config
    try:
        from pm_copilot_engine.agent.skill_utils import discover_all_skill_config_vars, resolve_skill_config_values
