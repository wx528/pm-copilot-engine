...
def _codex_curated_models() -> list[str]:
    ...
    """
    from pm_copilot_engine._internal.hermes_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def _xai_curated_models() -> list[str]:
    ...
    """
    try:
        from pm_copilot_engine.agent.models_dev import _load_disk_cache
...
try:
    from pm_copilot_engine.providers import list_providers as _list_providers_for_canonical
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from pm_copilot_engine._internal.hermes_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from pm_copilot_engine._internal.hermes_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from pm_copilot_engine._internal.hermes_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized == "bedrock":
        try:
            from pm_copilot_engine.agent.bedrock_adapter import bedrock_model_ids_or_none
...
def _fetch_anthropic_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    """
    try:
        from pm_copilot_engine.agent.anthropic_adapter import resolve_anthropic_token, _is_oauth_token
    except ImportError:
        return None
    ...
    if is_oauth:
        ...
        from pm_copilot_engine.agent.anthropic_adapter import _COMMON_BETAS, _OAUTH_ONLY_BETAS, _CONTEXT_1M_BETA
...
def validate_requested_model(
    ...
    *,
    ...
    if normalized == "bedrock":
        try:
            from pm_copilot_engine.agent.bedrock_adapter import discover_bedrock_models, resolve_bedrock_region
