...
from pm_copilot_engine._internal.hermes_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from pm_copilot_engine._internal.hermes_cli.build_info import get_build_sha
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if get_toolset_for_tool is None:
        from pm_copilot_engine.model_tools import get_toolset_for_tool
    ...
    try:
        from pm_copilot_engine.tools.mcp_tool import get_mcp_status
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.codex_runtime_switch import get_current_runtime
        from pm_copilot_engine._internal.hermes_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from pm_copilot_engine._internal.hermes_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
