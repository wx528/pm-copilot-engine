...
from pm_copilot_engine.utils import base_url_hostname
from pm_copilot_engine.hermes_constants import get_optional_skills_dir
...
from pm_copilot_engine._internal.hermes_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from pm_copilot_engine._internal.hermes_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from pm_copilot_engine._internal.hermes_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        from pm_copilot_engine.agent.auxiliary_client import get_available_vision_backends
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        _img_backend = None
        try:
            from pm_copilot_engine.agent.image_gen_registry import list_providers
            from pm_copilot_engine._internal.hermes_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            from pm_copilot_engine.agent.video_gen_registry import list_providers as _list_video_providers
            from pm_copilot_engine._internal.hermes_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if disabled_tools:
        ...
        from pm_copilot_engine.hermes_constants import display_hermes_home as _dhh
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from pm_copilot_engine._internal.hermes_cli.main import select_provider_and_model
    try:
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        from pm_copilot_engine.tools.managed_tool_gateway import is_managed_tool_gateway_ready
        from pm_copilot_engine.tools.tool_backend_helpers import normalize_modal_mode
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # HERMES_MAX_ITERATIONS from pm_copilot_engine.agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _write_slack_manifest_and_instruct():
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli.slack_cli import _build_full_manifest
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        try:
            from pm_copilot_engine.tools.lazy_deps import ensure as _lazy_ensure, feature_missing
...
def _setup_qqbot():
    ...
    from pm_copilot_engine._internal.hermes_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from pm_copilot_engine._internal.hermes_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if any_messaging:
        ...
        if service_running:
            if supports_systemd and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd:
                        ...
                        from pm_copilot_engine._internal.hermes_cli import gateway_windows
            ...
            from pm_copilot_engine.hermes_constants import is_container
            if is_container():
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from pm_copilot_engine._internal.hermes_cli.config
    ...
    if section_key == "model":
        ...
        from pm_copilot_engine._internal.hermes_cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from pm_copilot_engine._internal.hermes_cli.config import is_managed, managed_error
    if is_managed():
