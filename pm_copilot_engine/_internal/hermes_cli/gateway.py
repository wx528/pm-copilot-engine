...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from pm_copilot_engine._internal.hermes_cli.profiles import list_profiles
    except Exception:
        return processes
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    from pm_copilot_engine._internal.hermes_cli._subprocess_compat import (
...
def _gateway_list() -> None:
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
from pm_copilot_engine.hermes_constants import is_container, is_termux, is_wsl
...
def _spawn_detached_gateway() -> bool:
    ...
    """
    from pm_copilot_engine._internal.hermes_cli._subprocess_compat import windows_detach_popen_kwargs
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HERMES_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from pm_copilot_engine.hermes_constants import get_hermes_home as _ghh
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from pm_copilot_engine._internal.hermes_cli.telegram_managed_bot import (
...
def _setup_whatsapp():
    ...
    from pm_copilot_engine._internal.hermes_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from pm_copilot_engine._internal.hermes_cli.dingtalk_auth import dingtalk_qr_auth
...
def _builtin_setup_fn(key: str):
    ...
    """
    from pm_copilot_engine._internal.hermes_cli import setup as _s
