...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from pm_copilot_engine._internal.hermes_cli.config import _sanitize_env_lines
    except ImportError:
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    try:
        from pm_copilot_engine.agent.secret_sources.bitwarden import apply_bitwarden_secrets
    except ImportError:
        return
