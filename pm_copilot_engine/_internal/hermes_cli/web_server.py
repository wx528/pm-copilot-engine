...
from pm_copilot_engine.utils import env_var_enabled
...
try:
    ...
    try:
        from pm_copilot_engine.tools.lazy_deps import ensure as _lazy_ensure
...
# ---------------------------------------------------------------------------
from pm_copilot_engine._internal.hermes_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from pm_copilot_engine._internal.hermes_cli.dashboard_auth.middleware import gated_auth_middleware
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    """
    from pm_copilot_engine._internal.hermes_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from pm_copilot_engine._internal.hermes_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from pm_copilot_engine._internal.hermes_cli.models import _AGGREGATOR_PROVIDERS
...
async def get_status():
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.dashboard_auth import list_providers as _list_providers
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from pm_copilot_engine._internal.hermes_cli.auth import get_nous_auth_status
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from pm_copilot_engine._internal.hermes_cli.debug import build_debug_share
...
async def transcribe_audio_upload(payload: AudioTranscriptionRequest):
    ...
    try:
        ...
        from pm_copilot_engine.tools.transcription_tools import transcribe_audio
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    try:
        from pm_copilot_engine.tools.tts_tool import text_to_speech_tool
        loop = asyncio.get_running_loop()
...
async def get_profiles_sessions(
    ...
    from pm_copilot_engine.hermes_state import SessionDB
    from pm_copilot_engine._internal.hermes_cli import profiles as profiles_mod
...
def get_model_info(profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            from pm_copilot_engine.agent.model_metadata import get_model_context_length
        ...
        caps = {}
        try:
            from pm_copilot_engine.agent.models_dev import get_model_capabilities
...
def get_model_options(profile: Optional[str] = None):
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli.inventory import build_models_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            ...
            from pm_copilot_engine._internal.hermes_cli.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from pm_copilot_engine._internal.hermes_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() == "nous":
            try:
                from pm_copilot_engine._internal.hermes_cli.nous_subscription import apply_nous_managed_defaults
                from pm_copilot_engine._internal.hermes_cli.tools_config import _get_platform_tools
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from pm_copilot_engine._internal.hermes_cli.main import _auto_provider_name, _save_custom_provider
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        pass
    try:
        from pm_copilot_engine._internal.hermes_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    """
    try:
        from pm_copilot_engine.agent.anthropic_adapter import read_claude_code_credentials
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from pm_copilot_engine.agent.anthropic_adapter import _HERMES_OAUTH_FILE
        ...
        try:
            from pm_copilot_engine._internal.hermes_cli.auth import clear_provider_auth
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    try:
        from pm_copilot_engine.agent.credential_pool import (
            PooledCredential,
            load_pool,
            AUTH_TYPE_OAUTH,
            SOURCE_MANUAL,
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from pm_copilot_engine._internal.hermes_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from pm_copilot_engine._internal.hermes_cli.auth import _save_codex_tokens
...
async def get_logs(
    ...
    from pm_copilot_engine._internal.hermes_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from pm_copilot_engine.hermes_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def list_mcp_servers(profile: Optional[str] = None):
    from pm_copilot_engine._internal.hermes_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from pm_copilot_engine._internal.hermes_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from pm_copilot_engine._internal.hermes_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from pm_copilot_engine._internal.hermes_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli import mcp_catalog
    except Exception as exc:
...
async def list_webhooks():
    import pm_copilot_engine._internal.hermes_cli.webhook as wh
...
async def list_credential_pool():
    from pm_copilot_engine.agent.credential_pool import load_pool
    from pm_copilot_engine._internal.hermes_cli.auth import read_credential_pool
...
async def list_hooks():
    ...
    from pm_copilot_engine.agent import shell_hooks

    try:
        from pm_copilot_engine._internal.hermes_cli.plugins import VALID_HOOKS
...
def _installed_hub_identifiers(profile: Optional[str] = None) -> dict:
    ...
    """
    try:
        from pm_copilot_engine.tools.skills_hub import HubLockFile
...
async def list_skills_hub_sources(profile: Optional[str] = None):
    ...
    def _run():
        from pm_copilot_engine.tools.skills_hub import create_source_router
...
async def search_skills_hub(
    ...
    def _run():
        from pm_copilot_engine.tools.skills_hub import create_source_router, parallel_search_sources
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from pm_copilot_engine._internal.hermes_cli.skills_hub import _resolve_source_meta_and_bundle
        from pm_copilot_engine.tools.skills_hub import create_source_router
...
async def scan_skill_hub(identifier: str = ""):
    ...
    def _run():
        ...
        from pm_copilot_engine._internal.hermes_cli.skills_hub import _resolve_source_meta_and_bundle
        from pm_copilot_engine.tools.skills_hub import create_source_router, quarantine_bundle
        from pm_copilot_engine.tools.skills_guard import scan_skill, should_allow_install
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    """
    from pm_copilot_engine.hermes_constants import set_hermes_home_override, reset_hermes_home_override
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    """
    from pm_copilot_engine.hermes_constants import set_hermes_home_override, reset_hermes_home_override
    from pm_copilot_engine._internal.hermes_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    _resolve_profile_dir(name)
    try:
        from pm_copilot_engine._internal.hermes_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    from pm_copilot_engine.tools import skills_tool as _skills_tool
    from pm_copilot_engine.tools import skill_manager_tool as _skill_mgr
...
async def get_skills(profile: Optional[str] = None):
    from pm_copilot_engine.tools.skills_tool import _find_all_skills
    from pm_copilot_engine._internal.hermes_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
def _clear_skills_prompt_cache() -> None:
    ...
    """
    try:
        from pm_copilot_engine.agent.prompt_builder import clear_skills_system_prompt_cache
...
async def get_skill_content(name: str, profile: Optional[str] = None):
    ...
    from pm_copilot_engine.tools.skill_manager_tool import _find_skill
...
async def create_skill(body: SkillCreate):
    ...
    """
    from pm_copilot_engine.tools.skill_manager_tool import _create_skill
...
async def update_skill_content(body: SkillContentUpdate):
    ...
    from pm_copilot_engine.tools.skill_manager_tool import _edit_skill
...
async def get_toolsets(profile: Optional[str] = None):
    from pm_copilot_engine._internal.hermes_cli.tools_config import (
        _get_effective_configurable_toolsets,
        _get_platform_tools,
        _toolset_has_keys,
        gui_toolset_label,
    )
    from pm_copilot_engine.toolsets import resolve_toolset
...
if sys.platform.startswith("win"):
    try:
        from pm_copilot_engine._internal.hermes_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_client_reason(ws: "WebSocket") -> Optional[str]:
    ...
    so a "WS keeps closing" report can be diagnosed from pm_copilot_engine.agent.log without a
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from pm_copilot_engine._internal.hermes_cli.dashboard_auth.audit import AuditEvent, audit_log
        from pm_copilot_engine._internal.hermes_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    """
    from pm_copilot_engine._internal.hermes_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from pm_copilot_engine._internal.hermes_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    """
    from pm_copilot_engine._internal.hermes_cli.dashboard_auth.prefix import normalise_prefix
...
def _discover_dashboard_plugins() -> list:
    ...
    from pm_copilot_engine._internal.hermes_cli.plugins import get_bundled_plugins_dir
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from pm_copilot_engine._internal.hermes_cli.plugins_cmd import (
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from pm_copilot_engine._internal.hermes_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from pm_copilot_engine._internal.hermes_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from pm_copilot_engine._internal.hermes_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from pm_copilot_engine._internal.hermes_cli.plugins_cmd import dashboard_remove_user_plugin
...
from pm_copilot_engine._internal.hermes_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from pm_copilot_engine._internal.hermes_cli.dashboard_auth import list_providers
