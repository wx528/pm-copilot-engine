...
from pm_copilot_engine._internal.hermes_cli.config import get_project_root, get_hermes_home, get_env_path
...
from pm_copilot_engine._internal.hermes_cli.models import _HERMES_USER_AGENT
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from pm_copilot_engine._internal.hermes_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from pm_copilot_engine._internal.hermes_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli import __version__ as init_version
    except Exception:
        return
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli.service_manager import (
            S6ServiceManager,
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        ...
        from pm_copilot_engine._internal.hermes_cli.service_manager import detect_service_manager
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        from pm_copilot_engine.providers import list_providers
        from pm_copilot_engine.providers.base import ProviderProfile as _PP
        try:
            from pm_copilot_engine._internal.hermes_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    if ack_target:
        from pm_copilot_engine._internal.hermes_cli.security_advisories import (
            ADVISORIES,
            ack_advisory,
        )
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from pm_copilot_engine._internal.hermes_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from pm_copilot_engine._internal.hermes_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from pm_copilot_engine._internal.hermes_cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from pm_copilot_engine._internal.hermes_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from pm_copilot_engine._internal.hermes_cli.config import check_config_version, migrate_config
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from pm_copilot_engine.utils import atomic_yaml_write
        ...
        # bridge normally derives HERMES_MAX_ITERATIONS from pm_copilot_engine.agent.max_turns
        ...
        try:
            import yaml
            from pm_copilot_engine._internal.hermes_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
        ...
        try:
            from pm_copilot_engine._internal.hermes_cli.config import validate_config_structure
    ...
    if state_db_path.exists():
        try:
            ...
            from pm_copilot_engine.hermes_state import is_malformed_db_error, repair_state_db_schema
    ...
    try:
        from pm_copilot_engine.hermes_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                from pm_copilot_engine.tools.browser_tool import (
                    _chromium_installed,
                    _is_camofox_mode,
                    _get_cloud_provider,
                    _get_cdp_override,
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            if base and base.rstrip("/").endswith("/anthropic"):
                from pm_copilot_engine.agent.auxiliary_client import _to_openai_base_url
    ...
    try:
        ...
        from pm_copilot_engine.model_tools import check_tool_availability, TOOLSET_REQUIREMENTS
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
