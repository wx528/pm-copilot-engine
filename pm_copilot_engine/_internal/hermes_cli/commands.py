...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def _collect_gateway_skill_entries(
    ...
    try:
        from pm_copilot_engine.agent.skill_commands import get_skill_commands
        from pm_copilot_engine.tools.skills_tool import SKILLS_DIR
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from pm_copilot_engine._internal.hermes_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
