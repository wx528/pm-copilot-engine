...
from pm_copilot_engine.tools.mcp_tool import _ENV_VAR_PATTERN
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from pm_copilot_engine._internal.hermes_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    """
    from pm_copilot_engine.tools.mcp_tool import _interpolate_env_vars
...
def _probe_single_server(
    ...
    """
    from pm_copilot_engine.tools.mcp_tool import (
        _ensure_mcp_loop,
        _run_on_mcp_loop,
        _connect_server,
        _stop_mcp_loop,
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    """
    try:
        from pm_copilot_engine.tools.mcp_oauth import HermesTokenStorage
...
def cmd_mcp_add(args):
    ...
    if url and auth_type == "oauth":
        ...
        oauth_ok = False
        try:
            from pm_copilot_engine.tools.mcp_oauth_manager import get_manager
...
def mcp_command(args):
    ...
    if action == "serve":
        from pm_copilot_engine.mcp_serve import run_mcp_server
    ...
    if action == "picker":
        from pm_copilot_engine._internal.hermes_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from pm_copilot_engine._internal.hermes_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from pm_copilot_engine._internal.hermes_cli.mcp_picker import install_by_name
        import sys as _sys
