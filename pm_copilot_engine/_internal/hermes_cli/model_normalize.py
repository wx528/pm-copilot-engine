...
def _normalize_provider_alias(provider_name: str) -> str:
    ...
    if not raw:
        return raw
    try:
        from pm_copilot_engine._internal.hermes_cli.models import normalize_provider
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from pm_copilot_engine._internal.hermes_cli.models import normalize_copilot_model_id
