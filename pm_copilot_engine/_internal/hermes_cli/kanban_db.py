...
from pm_copilot_engine.toolsets import get_toolset_names
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                from pm_copilot_engine.hermes_state import apply_wal_with_fallback
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from pm_copilot_engine._internal.hermes_cli.profiles import normalize_profile_name
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from pm_copilot_engine._internal.hermes_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from pm_copilot_engine._internal.hermes_cli.profiles import profile_exists as _pe
...
def _resolve_worker_cli_toolsets(hermes_home: Optional[str]) -> Optional[list[str]]:
    ...
    if not hermes_home:
        return None
    try:
        from pm_copilot_engine.hermes_constants import reset_hermes_home_override, set_hermes_home_override
        from pm_copilot_engine._internal.hermes_cli.config import load_config
