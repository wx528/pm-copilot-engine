...
def _invoke_middleware(kind: str, **kwargs: Any) -> List[Any]:
    from pm_copilot_engine._internal.hermes_cli.plugins import invoke_middleware

    return invoke_middleware(kind, **middleware_payload(**kwargs))


def _has_middleware(kind: str) -> bool:
    from pm_copilot_engine._internal.hermes_cli.plugins import has_middleware

    return has_middleware(kind)


def _get_middleware_callbacks(kind: str) -> List[Callable]:
    from pm_copilot_engine._internal.hermes_cli.plugins import get_plugin_manager
