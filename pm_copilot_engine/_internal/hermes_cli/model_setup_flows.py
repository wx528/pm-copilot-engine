...
in main.py re-imports them (``from pm_copilot_engine._internal.hermes_cli.model_setup_flows import *``-style
...
imported lazily inside the flows (``from pm_copilot_engine._internal.hermes_cli.main import ...`` resolves at
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from pm_copilot_engine._internal.hermes_cli.main import _prompt_api_key
    ...
    from pm_copilot_engine._internal.hermes_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from pm_copilot_engine._internal.hermes_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from pm_copilot_engine._internal.hermes_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from pm_copilot_engine._internal.hermes_cli.codex_models import get_codex_model_ids
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from pm_copilot_engine._internal.hermes_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from pm_copilot_engine._internal.hermes_cli.auth import (
    ...
    from pm_copilot_engine._internal.hermes_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    if not status.get("logged_in"):
        try:
            from pm_copilot_engine.agent.google_oauth import resolve_project_id_from_env, start_oauth_flow
...
def _model_flow_custom(config):
    ...
    """
    from pm_copilot_engine._internal.hermes_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from pm_copilot_engine._internal.hermes_cli.auth import _save_model_choice, deactivate_provider
    from pm_copilot_engine._internal.hermes_cli.config import get_env_value, load_config, save_config
    ...
    from pm_copilot_engine._internal.hermes_cli.models import probe_api_models
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from pm_copilot_engine._internal.hermes_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from pm_copilot_engine._internal.hermes_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from pm_copilot_engine._internal.hermes_cli import azure_detect
    ...
    if use_entra:
        try:
            from pm_copilot_engine.agent.azure_identity_adapter import (
                EntraIdentityConfig,
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from pm_copilot_engine._internal.hermes_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from pm_copilot_engine._internal.hermes_cli.auth import _save_model_choice, deactivate_provider
    from pm_copilot_engine._internal.hermes_cli.config import load_config, save_config
    from pm_copilot_engine._internal.hermes_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from pm_copilot_engine._internal.hermes_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from pm_copilot_engine._internal.hermes_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from pm_copilot_engine._internal.hermes_cli.config import save_env_value, load_config, save_config
    from pm_copilot_engine._internal.hermes_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from pm_copilot_engine._internal.hermes_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from pm_copilot_engine._internal.hermes_cli.copilot_auth import validate_copilot_token
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from pm_copilot_engine._internal.hermes_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from pm_copilot_engine._internal.hermes_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from pm_copilot_engine._internal.hermes_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock(config, current_model=""):
    ...
    try:
        from pm_copilot_engine.agent.bedrock_adapter import (
            has_aws_credentials,
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        try:
            from pm_copilot_engine.agent.gemini_native_adapter import probe_gemini_tier
        except Exception:
    ...
    if provider_id == "lmstudio":
        ...
        from pm_copilot_engine._internal.hermes_cli.models import fetch_lmstudio_models
        ...
        from pm_copilot_engine._internal.hermes_cli.models import fetch_ollama_cloud_models
        ...
        if live_models:
            ...
            mdev_models: list = []
            try:
                from pm_copilot_engine.agent.models_dev import list_agentic_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from pm_copilot_engine._internal.hermes_cli.main import _run_anthropic_oauth_flow
    from pm_copilot_engine._internal.hermes_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
