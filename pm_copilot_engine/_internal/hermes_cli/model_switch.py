...
from pm_copilot_engine.agent.models_dev import (
    ModelCapabilities,
    ModelInfo,
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from pm_copilot_engine._internal.hermes_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        from pm_copilot_engine._internal.hermes_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from pm_copilot_engine._internal.hermes_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
        ...
        if not new_model:
            if pdef.base_url:
                from pm_copilot_engine._internal.hermes_cli.runtime_provider import _auto_detect_local_model
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from pm_copilot_engine._internal.hermes_cli.providers import resolve_user_provider as _ruser
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from pm_copilot_engine._internal.hermes_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from pm_copilot_engine._internal.hermes_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    def _has_aws_sdk_creds_for_listing(slug: str) -> bool:
        ...
        if slug_norm != current_norm:
            return False
        try:
            from pm_copilot_engine.agent.bedrock_adapter import has_aws_credentials
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from pm_copilot_engine._internal.hermes_cli.auth import _load_auth_store
    ...
    from pm_copilot_engine._internal.hermes_cli.providers import HERMES_OVERLAYS
    from pm_copilot_engine._internal.hermes_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from pm_copilot_engine._internal.hermes_cli.auth import get_provider_auth_state as _nous_state
    ...
    # with `hermes model`).
    try:
        from pm_copilot_engine._internal.hermes_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
...
def list_picker_providers(
    ...
    """
    from pm_copilot_engine._internal.hermes_cli.models import fetch_openrouter_models
