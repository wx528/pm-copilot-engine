...
from pm_copilot_engine._internal.hermes_cli.config import get_env_value, load_config
...
def _local_browser_runnable() -> bool:
    ...
    if not _has_agent_browser():
        return False
    try:
        from pm_copilot_engine.tools.browser_tool import _chromium_installed, _using_lightpanda_engine
    except Exception:
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("HERMES_LOCAL_STT_COMMAND"):
        return True
    try:
        from pm_copilot_engine.tools.transcription_tools import _HAS_FASTER_WHISPER
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    if changed:
        from pm_copilot_engine._internal.hermes_cli.config import save_config
