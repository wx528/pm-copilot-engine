...
def _get_custom_provider_names() -> list:
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.config import get_compatible_custom_providers, load_config
...
def _provider_base_url(provider: str) -> str:
    ...
    if provider.startswith(CUSTOM_POOL_PREFIX):
        from pm_copilot_engine.agent.credential_pool import _get_custom_provider_config
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        from pm_copilot_engine.agent import anthropic_adapter as anthropic_mod
    ...
    if provider == "google-gemini-cli":
        from pm_copilot_engine.agent.google_oauth import run_gemini_oauth_login_pure
...
def auth_remove_command(args) -> None:
    ...
    from pm_copilot_engine.agent.credential_sources import find_removal_step
    from pm_copilot_engine._internal.hermes_cli.auth import suppress_credential_source
...
def _interactive_auth() -> None:
    ...
    try:
        from pm_copilot_engine.agent.bedrock_adapter import has_aws_credentials, resolve_aws_auth_env_var, resolve_bedrock_region
