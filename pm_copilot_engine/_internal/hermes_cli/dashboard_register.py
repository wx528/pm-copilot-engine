...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _print_post_register_hint(
    *,
    ...
    from pm_copilot_engine._internal.hermes_cli.config import get_env_path
...
def cmd_dashboard_register(args) -> None:
    ...
    from pm_copilot_engine._internal.hermes_cli.auth import AuthError, resolve_nous_access_token
    from pm_copilot_engine._internal.hermes_cli.config import get_env_value, is_managed, save_env_value
