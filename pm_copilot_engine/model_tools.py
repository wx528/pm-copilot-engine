...
from pm_copilot_engine.tools.registry import discover_builtin_tools, registry
from pm_copilot_engine.toolsets import resolve_toolset, validate_toolset
...
def get_tool_definitions(
    ...
    if quiet_mode:
        try:
            from pm_copilot_engine._internal.hermes_cli.config import get_config_path
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        from pm_copilot_engine.toolsets import get_all_toolsets
    ...
    if "execute_code" in available_tool_names:
        from pm_copilot_engine.tools.code_execution_tool import SANDBOX_ALLOWED_TOOLS, build_execute_code_schema, _get_execution_mode
    ...
    for discord_tool_name in _discord_schema_fns:
        if discord_tool_name in available_tool_names:
            try:
                from pm_copilot_engine.tools import discord_tool as _dt
    ...
    try:
        from pm_copilot_engine.tools.schema_sanitizer import sanitize_tool_schemas
    ...
    try:
        from pm_copilot_engine.tools.tool_search import assemble_tool_defs, load_config as _load_ts_config
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    _ts_mod = None
    try:
        from pm_copilot_engine.tools import tool_search as _ts_mod  # noqa: F401
    except Exception:
        _ts_mod = None
    ...
    try:
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from pm_copilot_engine.tools.file_tools import notify_other_tool_call
