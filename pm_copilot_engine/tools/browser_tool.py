...
Usage:
    from pm_copilot_engine.tools.browser_tool import browser_navigate, browser_snapshot, browser_click
...
from pm_copilot_engine.agent.auxiliary_client import call_llm
from pm_copilot_engine.hermes_constants import get_hermes_home
from pm_copilot_engine.utils import env_int, is_truthy_value
from pm_copilot_engine._internal.hermes_cli.config import cfg_get

try:
    from pm_copilot_engine.tools.website_policy import check_website_access
except Exception:
    check_website_access = lambda url: None  # noqa: E731 — fail-open if policy module unavailable

try:
    from pm_copilot_engine.tools.url_safety import (
...
from pm_copilot_engine.agent.browser_provider import BrowserProvider as CloudBrowserProvider  # noqa: F401  (legacy alias)
from pm_copilot_engine.agent.browser_registry import (  # noqa: F401  (test-patchable surface)
...
from pm_copilot_engine.tools.tool_backend_helpers import normalize_browser_cloud_provider
# Camofox local anti-detection browser backend (optional).
# When CAMOFOX_URL is set, all browser operations route through the
# camofox REST API instead of the agent-browser CLI.
try:
    from pm_copilot_engine.tools.browser_camofox import is_camofox_mode as _is_camofox_mode
except ImportError:
...
def _get_dialog_policy_config() -> Tuple[str, float]:
    ...
    from pm_copilot_engine.tools.browser_supervisor import (
...
def _ensure_cdp_supervisor(task_id: str) -> None:
    ...
    if not cdp_url:
        return
    try:
        from pm_copilot_engine.tools.browser_supervisor import SUPERVISOR_REGISTRY  # type: ignore[import-not-found]
...
from pm_copilot_engine.hermes_constants import is_termux as _is_termux_environment
...
def _reap_orphaned_browser_sessions():
    ...
    for socket_dir in socket_dirs:
        ...
        try:
            from pm_copilot_engine.tools.process_registry import ProcessRegistry
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    import urllib.parse
    from pm_copilot_engine.agent.redact import _PREFIX_RE
    ...
    if _is_camofox_mode():
        from pm_copilot_engine.tools.browser_camofox import camofox_navigate
...
def browser_snapshot(
    ...
    """
    if _is_camofox_mode():
        from pm_copilot_engine.tools.browser_camofox import camofox_snapshot
...
def browser_click(ref: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from pm_copilot_engine.tools.browser_camofox import camofox_click
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from pm_copilot_engine.tools.browser_camofox import camofox_type
...
def browser_scroll(direction: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from pm_copilot_engine.tools.browser_camofox import camofox_scroll
...
def browser_back(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from pm_copilot_engine.tools.browser_camofox import camofox_back
...
def browser_press(key: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from pm_copilot_engine.tools.browser_camofox import camofox_press
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from pm_copilot_engine.tools.browser_camofox import camofox_console
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    from pm_copilot_engine.tools.browser_camofox import _ensure_tab, _post
    try:
...
def browser_get_images(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from pm_copilot_engine.tools.browser_camofox import camofox_get_images
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    """
    if _is_camofox_mode():
        from pm_copilot_engine.tools.browser_camofox import camofox_vision
        return camofox_vision(question, annotate, task_id)

    import base64
    import uuid as uuid_mod
    from pm_copilot_engine.hermes_constants import get_hermes_dir
    ...
    try:
        ...
        from pm_copilot_engine.tools.vision_tools import (
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if _is_camofox_mode():
        try:
            from pm_copilot_engine.tools.browser_camofox import camofox_close, camofox_soft_cleanup
...
def check_browser_vision_requirements() -> bool:
    ...
    if not check_browser_requirements():
        return False
    try:
        from pm_copilot_engine.tools.vision_tools import check_vision_requirements
    except ImportError:
        return False
...
if __name__ == "__main__":
    ...
    print("\n💡 Usage:")
    print("  from pm_copilot_engine.tools.browser_tool import browser_navigate, browser_snapshot")
