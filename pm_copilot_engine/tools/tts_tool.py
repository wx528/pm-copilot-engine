...
Usage:
    from pm_copilot_engine.tools.tts_tool import text_to_speech_tool, check_tts_requirements
...
def get_env_value(name, default=None):
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli.config import get_env_value as _get_env_value
    except ImportError:
        return os.getenv(name, default)
    value = _get_env_value(name)
    return default if value is None else value
from pm_copilot_engine.tools.managed_tool_gateway import resolve_managed_tool_gateway
...
from pm_copilot_engine.tools.xai_http import hermes_xai_user_agent
...
def _import_elevenlabs():
    ...
    """
    try:
        from pm_copilot_engine.tools.lazy_deps import FeatureUnavailable, ensure
...
def _import_mistral_client():
    ...
    """
    try:
        from pm_copilot_engine.tools.lazy_deps import ensure
...
def _dispatch_to_plugin_provider(
    ...
    if _is_command_provider_config(_get_named_provider_config(tts_config, key)):
        return None
    try:
        from pm_copilot_engine.agent.tts_registry import get_provider
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    from pm_copilot_engine.tools.xai_http import resolve_xai_http_credentials
    ...
    try:
        ...
        def _play_via_tempfile(audio_iter, stop_evt):
            ...
            try:
                ...
                from pm_copilot_engine.tools.voice_mode import play_audio_file
