...
Usage:
    from pm_copilot_engine.tools.skills_tool import skills_list, skill_view, check_skills_requirements
...
from pm_copilot_engine.agent.skill_utils import EXCLUDED_SKILL_DIRS as _EXCLUDED_SKILL_DIRS
...
def _skill_lookup_path_error(name: str) -> Optional[str]:
    ...
    """
    from pm_copilot_engine.tools.path_security import has_traversal_component
...
def skill_matches_platform(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from pm_copilot_engine.agent.skill_utils import skill_matches_platform as _impl
...
def skill_matches_environment(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from pm_copilot_engine.agent.skill_utils import skill_matches_environment as _impl
    return _impl(frontmatter)
...
def _parse_frontmatter(content: str) -> Tuple[Dict[str, Any], str]:
    ...
    as a public re-export so existing callers don't need updating.
    """
    from pm_copilot_engine.agent.skill_utils import parse_frontmatter
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    try:
        from pm_copilot_engine.agent.skill_utils import get_external_skills_dirs
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    """
    from pm_copilot_engine.agent.skill_utils import get_external_skills_dirs, iter_skill_index_files
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from pm_copilot_engine._internal.hermes_cli.plugins import _get_disabled_plugins, get_plugin_manager
    ...
    if preprocess:
        try:
            from pm_copilot_engine.agent.skill_preprocessing import preprocess_skill_content
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            from pm_copilot_engine.agent.skill_utils import is_valid_namespace, parse_qualified_name
        ...
        from pm_copilot_engine.agent.skill_utils import iter_skill_index_files
        ...
        if file_path and skill_dir:
            from pm_copilot_engine.tools.path_security import validate_within_dir, has_traversal_component
        ...
        if available_env_names:
            try:
                from pm_copilot_engine.tools.env_passthrough import register_env_passthrough
        ...
        if required_cred_files_raw:
            try:
                from pm_copilot_engine.tools.credential_files import register_credential_files
...
def _skill_view_with_bump(args, **kw):
    ...
    try:
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            ...
            if resolved:
                from pm_copilot_engine.tools.skill_usage import bump_use, bump_view
