...
from plugins.web.firecrawl.provider import (
    ...
    _get_firecrawl_client,  # noqa: F401  # re-exported for tests that `from pm_copilot_engine.tools.web_tools import _get_firecrawl_client`
...
from pm_copilot_engine.tools.debug_helpers import DebugSession
...
from pm_copilot_engine.tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    build_vendor_gateway_url,
    peek_nous_access_token as _peek_nous_access_token,
    read_nous_access_token as _read_nous_access_token,
    resolve_managed_tool_gateway,
)
from pm_copilot_engine.tools.tool_backend_helpers import (  # noqa: F401
    managed_nous_tools_enabled,
    nous_tool_gateway_unavailable_message,
    prefers_gateway,
)
from pm_copilot_engine.tools.url_safety import async_is_safe_url, normalize_url_for_request
import sys
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        try:
            from pm_copilot_engine.tools.xai_http import has_xai_credentials
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        from pm_copilot_engine.agent.auxiliary_client import get_auxiliary_extra_body
        from pm_copilot_engine.agent.portal_tags import nous_portal_tags
...
def _ensure_web_plugins_loaded() -> None:
    ...
    try:
        from pm_copilot_engine.agent.web_search_registry import get_provider
...
def _register_bundled_web_providers_directly() -> None:
    ...
    from pm_copilot_engine.agent.web_search_registry import register_provider
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        from pm_copilot_engine.agent.web_search_registry import (
