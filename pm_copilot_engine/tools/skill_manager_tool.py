...
from pm_copilot_engine.utils import atomic_replace, is_truthy_value
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    ``_find_skill`` first).
    """
    from pm_copilot_engine.agent.skill_utils import get_all_skills_dirs
...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    """
    try:
        from pm_copilot_engine.tools import skill_usage
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    {"path": Path} or None.
    """
    from pm_copilot_engine.agent.skill_utils import get_all_skills_dirs, is_excluded_skill_path
...
def _skill_not_found_error(name: str, suffix: str = "") -> str:
    ...
    """
    from pm_copilot_engine.agent.file_safety import _resolve_active_profile_name
...
def _resolve_skill_target(skill_dir: Path, file_path: str) -> Tuple[Optional[Path], Optional[str]]:
    ...
    from pm_copilot_engine.tools.path_security import validate_within_dir
...
def _patch_skill(
    ...
    from pm_copilot_engine.tools.fuzzy_match import fuzzy_find_and_replace
    ...
    if match_error:
        ...
        err_msg = match_error
        try:
            from pm_copilot_engine.tools.fuzzy_match import format_no_match_hint
...
def skill_manage(
    ...
    if result.get("success"):
        ...
        try:
            from pm_copilot_engine.tools.skill_usage import bump_patch, forget, mark_agent_created
            from pm_copilot_engine.tools.skill_provenance import is_background_review
            if action == "create":
