...
Usage:
    from pm_copilot_engine.tools.file_operations import ShellFileOperations
    from pm_copilot_engine.tools.terminal_tool import _active_environments
...
from pm_copilot_engine.tools.binary_extensions import BINARY_EXTENSIONS

from pm_copilot_engine.agent.file_safety import (
...
def normalize_read_pagination(offset: Any = DEFAULT_READ_OFFSET,
    ...
    """
    from pm_copilot_engine.tools.tool_output_limits import get_max_lines
...
class ShellFileOperations(FileOperations):
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        """
        from pm_copilot_engine.tools.tool_output_limits import get_max_line_length
    ...
    def patch_v4a(self, patch_content: str) -> PatchResult:
        ...
        """
        # Import patch parser
        from pm_copilot_engine.tools.patch_parser import parse_v4a_patch, apply_v4a_operations
    ...
    def _lsp_local_only(self) -> bool:
        ...
        if env is None:
            ...
            return False
        try:
            from pm_copilot_engine.tools.environments.local import LocalEnvironment
    ...
    def _lsp_handles_extension(self, ext: str) -> bool:
        ...
        if not ext:
            return False
        try:
            from pm_copilot_engine.agent.lsp.servers import SERVERS
        except Exception:  # noqa: BLE001
            return False
        ext_lower = ext.lower()
        for srv in SERVERS:
    ...
    def _lsp_will_handle(self, path: str) -> bool:
        ...
        if not self._lsp_local_only():
            return False
        try:
            from pm_copilot_engine.agent.lsp import get_service
        except Exception:  # noqa: BLE001
            return False
        try:
            svc = get_service()
    ...
    def _maybe_lsp_diagnostics(
        ...
        *,
        ...
        if pre_content is not None and post_content is not None and pre_content != post_content:
            try:
                from pm_copilot_engine.agent.lsp.range_shift import build_line_shift
        ...
        if not diagnostics:
            return ""
        try:
            from pm_copilot_engine.agent.lsp.reporter import report_for_file, truncate
