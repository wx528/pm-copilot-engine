...
Import chain (circular-import safe):
    tools/registry.py  (no imports from pm_copilot_engine.model_tools or tool files)
           ^
    tools/*.py  (import from pm_copilot_engine.tools.registry at module level)
           ^
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    module_names = [
        f"pm_copilot_engine.tools.{path.stem}"
...
class ToolRegistry:
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> str:
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        try:
            ...
            try:
                from pm_copilot_engine.model_tools import _sanitize_tool_error
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from pm_copilot_engine.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
...
#
# Usage:
#   from pm_copilot_engine.tools.registry import registry, tool_error, tool_result
#
