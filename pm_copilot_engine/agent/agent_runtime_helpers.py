...
from pm_copilot_engine.agent.tool_dispatch_helpers import _trajectory_normalize_msg, make_tool_result_message
from pm_copilot_engine.agent.trajectory import convert_scratchpad_to_think
from pm_copilot_engine.agent.credential_pool import STATUS_EXHAUSTED
from pm_copilot_engine.agent.error_classifier import FailoverReason
from pm_copilot_engine.utils import base_url_host_matches, base_url_hostname, env_var_enabled, atomic_json_write
    ...
    *,
    ...
    if current_provider and pool_provider and current_provider != pool_provider:
        ...
        if current_provider == "custom" and pool_provider.startswith("custom:"):
            try:
                from pm_copilot_engine.agent.credential_pool import get_custom_provider_pool_key
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    from pm_copilot_engine.agent.auxiliary_client import _validate_base_url, _validate_proxy_env_urls
    ...
    if agent.provider == "google-gemini-cli" or str(client_kwargs.get("base_url", "")).startswith("cloudcode-pa://"):
        from pm_copilot_engine.agent.gemini_cloudcode_adapter import GeminiCloudCodeClient
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from pm_copilot_engine._internal.hermes_cli.providers import determine_api_mode
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from pm_copilot_engine._internal.hermes_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.middleware import apply_tool_request_middleware
    ...
    if not pre_tool_block_checked:
        try:
            from pm_copilot_engine._internal.hermes_cli.plugins import get_pre_tool_call_block_message
    ...
    if block_message is not None:
        ...
        try:
            from pm_copilot_engine.model_tools import _emit_post_tool_call_hook
    ...
    if function_name == "todo":
        def _execute(next_args: dict) -> Any:
            from pm_copilot_engine.tools.todo_tool import todo_tool as _todo_tool
        ...
        def _execute(next_args: dict) -> Any:
            ...
            from pm_copilot_engine.tools.session_search_tool import session_search as _session_search
            return _finish_agent_tool(
                _session_search(
        ...
        def _execute(next_args: dict) -> Any:
            ...
            from pm_copilot_engine.tools.memory_tool import memory_tool as _memory_tool
            result = _memory_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from pm_copilot_engine.tools.clarify_tool import clarify_tool as _clarify_tool
            return _finish_agent_tool(
                _clarify_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from pm_copilot_engine.tools.read_terminal_tool import read_terminal_tool as _read_terminal_tool
            return _finish_agent_tool(
                _read_terminal_tool(
    ...
    from pm_copilot_engine._internal.hermes_cli.middleware import run_tool_execution_middleware
