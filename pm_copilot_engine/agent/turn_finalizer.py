...
Module ``logger`` is imported lazily inside the body (``from pm_copilot_engine.agent.conversation_loop import logger``) so this module never imports
...
def finalize_turn(
    ...
    *,
    ...
    """
    from pm_copilot_engine.agent.conversation_loop import logger
    ...
    if final_response is None and (
        ...
        if _kanban_task:
            try:
                from pm_copilot_engine._internal.hermes_cli import kanban_db as _kb
                _conn = _kb.connect()
                try:
