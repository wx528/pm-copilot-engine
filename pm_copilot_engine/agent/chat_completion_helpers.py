...
from pm_copilot_engine._internal.hermes_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from pm_copilot_engine.hermes_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
from pm_copilot_engine.agent.error_classifier import FailoverReason
from pm_copilot_engine.agent.model_metadata import is_local_endpoint
from pm_copilot_engine.agent.message_sanitization import (
    _sanitize_surrogates,
    _repair_tool_call_arguments,
)
from pm_copilot_engine.tools.terminal_tool import is_persistent_env
from pm_copilot_engine.utils import base_url_host_matches, base_url_hostname, env_int
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # Request-local cancellation flag. Distinct from pm_copilot_engine.agent._interrupt_requested
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    try:
        from pm_copilot_engine.agent.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE
    ...
    if (_is_or or _is_nous) and "claude" in (agent.model or "").lower():
        try:
            from pm_copilot_engine.agent.anthropic_adapter import _get_anthropic_max_output
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        if fb_api_mode == "anthropic_messages":
            ...
            from pm_copilot_engine.agent.anthropic_adapter import build_anthropic_client, resolve_anthropic_token, _is_oauth_token
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        summary_extra_body = {}
        try:
            from pm_copilot_engine.agent.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE as _OMIT_TEMP
        except Exception:
        ...
        if _is_nous:
            from pm_copilot_engine.agent.portal_tags import nous_portal_tags as _portal_tags
