...
from pm_copilot_engine._internal.hermes_cli.config import load_env
from pm_copilot_engine.agent.credential_persistence import (
    is_borrowed_credential_source,
    sanitize_borrowed_credential_payload,
)
import pm_copilot_engine._internal.hermes_cli.auth as auth_mod
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                from pm_copilot_engine.agent.anthropic_adapter import refresh_anthropic_oauth_pure
                ...
                if entry.source == "claude_code":
                    try:
                        from pm_copilot_engine.agent.anthropic_adapter import _write_claude_code_credentials
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        from pm_copilot_engine.agent.anthropic_adapter import read_claude_code_credentials, read_hermes_oauth_credentials
        ...
        try:
            from pm_copilot_engine._internal.hermes_cli.auth import resolve_qwen_runtime_credentials
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from pm_copilot_engine._internal.hermes_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from pm_copilot_engine._internal.hermes_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from pm_copilot_engine._internal.hermes_cli.env_loader import get_secret_source
