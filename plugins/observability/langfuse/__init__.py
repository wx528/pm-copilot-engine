...
def _usage_and_cost(response: Any, *, provider: str, api_mode: str, model: str, base_url: str) -> tuple[dict[str, int], dict[str, float]]:
    ...
    try:
        ...
        if cost.amount_usd is not None:
            ...
            try:
                from pm_copilot_engine.agent.usage_pricing import get_pricing_entry
...
def on_post_llm_call(*, task_id: str = "", session_id: str = "", provider: str = "", base_url: str = "",
    ...
    if getattr(response, "usage", None) is not None:
        ...
        try:
            from pm_copilot_engine.agent.usage_pricing import CanonicalUsage, estimate_usage_cost, get_pricing_entry
            from decimal import Decimal
            _ONE_M = Decimal("1000000")
            _cu = CanonicalUsage(
                input_tokens=_input,
