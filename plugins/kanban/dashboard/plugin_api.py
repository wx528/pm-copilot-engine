...
from pm_copilot_engine._internal.hermes_cli import kanban_db
from pm_copilot_engine._internal.hermes_cli import kanban_diagnostics as kd
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli import web_server as _ws
    except Exception:
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from pm_copilot_engine._internal.hermes_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from pm_copilot_engine._internal.hermes_cli.kanban import _check_dispatcher_presence
...
def specify_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from pm_copilot_engine._internal.hermes_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from pm_copilot_engine.hermes_constants import get_hermes_home  # type: ignore
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    """
    try:
        from pm_copilot_engine._internal.hermes_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from pm_copilot_engine._internal.hermes_cli import kanban_decompose  # noqa: WPS433 (intentional)
